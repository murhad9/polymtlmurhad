...
int main() {
  ...
  tests.push_back(sortieAttendueAuteurs == auteurString &&
                  resultatLectureAuteurs == true &&
  ...
  tests.push_back(sortieAttendueLibrairie == librairieString &&
                  resultatLectureFilms == true && resultatLectureRestrictions &&
                  librairie.getNbFilms() == 9);
#else
  ...
  for (auto i = gestionnaireAuteurs.getNbAuteurs();
       i < GestionnaireAuteurs::NB_AUTEURS_MAX; i++) {
  ...
  const Auteur *auteurInexsistant =
      gestionnaireAuteurs.chercherAuteur("qwerty");
  ...
  for (unsigned int i = 0; i < test8NbFilmsAjouter; i++) {
    librairie.ajouterFilm(new Film("test", 1, Film::Genre::Action, Pays::Canada,
                                   false, &auteurTest8));
  }
  // Ajouter un film nullptr ne devrait rien faire
  librairie.ajouterFilm(nullptr);
  tests.push_back(librairie.getNbFilms() ==
                  nbFilmsAvantTest8 + test8NbFilmsAjouter);
#else
  tests.push_back(false);
#endif
  ...
  tests.push_back(librairie.getNbFilms() + test8NbFilmsAjouter ==
                      nbFilmsAvantTest9 &&
  ...
  tests.push_back(utilisateurNonPremium.getNbFilmsVus() ==
                  Utilisateur::NB_FILMS_GRATUITS);
  ...
  Film film3("film3", 1995, Film::Genre::Aventure, Pays::France, false,
             &auteur);
  auto result1 = utilisateurPremium2.regarderFilm(film2);
  auto result2 = utilisateurPremium2.regarderFilm(film3);
  tests.push_back(utilisateurPremium2.getNbFilmsVus() == 1 && !result1 &&
                  result2);
  ...
  std::vector<float> ponderationTests = {1,   2,   1,   0.5, 1,
                                         0.5, 0.5, 0.5, 0.5, 0.5};
